 ...
 * ----------------------------------------------------------------------------------------- */
/**
 * Template_Name: Social Entry
 * Description: This template is designated for a single social entry
 */
...
class SWPTemplateSample {
	...
	// DISPLAY VIDEO ACTUAL
	public function swp_social_entry_actual() {

		?><section class="single-actual space-lrg-bottom"><?php

		// Twitter
		$display_twitter = get_post_meta( get_the_ID(), "twitter_oembed", TRUE );
		if( $display_twitter ) {
			?><div><?php
			the_field('twitter_oembed');
			?></div><?php
		}

		// Instagram
		$display_instagram = get_post_meta( get_the_ID(), "instagram_oembed", TRUE );
		if( $display_instagram ) {
			?><div><?php
			the_field('instagram_oembed');
			?></div><?php
		}

		// Facebook
		$display_facebook = get_post_meta( get_the_ID(), "facebook_oembed", TRUE );
		if( $display_facebook ) {
			?><div><?php
			the_field('facebook_oembed');
			?></div><?php
		}
	...
	// DISPLAY SOCIAL INFO
	public function swp_social_entry_info() {

		?><section class="single-info"><?php
		echo '<div class="item-social">'.do_shortcode( get_post_meta( get_the_ID(), social_embed, TRUE ) ).'</div>';
	...
	// DISPLAY SOCIAL GRID TEMPLATE
	/*public function swp_social_related_function() {
	...
	// DISPLAY SOCIAL NEXT ITEM
	public function swp_social_next() {
	...
	// GRID
	public function swp_social_related_function_2() {
    	...
    	echo '<div class="feature-pretitle">MORE SOCIAL MEDIA</div>';
        $more = array(
						'tag-open'		=> '<div class="grid-feature grid-2col feature-related">', // opening container tag here
						'tag-close'		=> '</div>', // closing container tag here
					);
	...
	public function __construct() {
		...
		if( !is_admin() ) {
			...
			add_action( 'genesis_entry_content', array( $this, 'swp_social_entry_actual' ) );
			//add_action( 'genesis_entry_content', array( $this, 'swp_social_entry_info' ) );
			add_action( 'genesis_entry_content', 'genesis_do_post_content' );
			//add_action( 'genesis_entry_content', array( $this, 'swp_social_related_function' ) );
			add_action( 'genesis_entry_content', array( $this, 'swp_social_related_function_2' ) );
			...
			add_action( 'genesis_before_sidebar_widget_area', array( $this, 'swp_social_next' ) );
