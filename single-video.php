...
class SWPTemplateSample {
	...
	// DISPLAY VIDEO PIC
	public function swp_video_entry_pic() {

	    // IMAGE
        $swp_field = get_post_meta( get_the_ID(), "pic", TRUE );
        if ( $swp_field ) {
            ?><div class="item-pic"><a href="<?php echo get_the_permalink(); ?>"><?php echo wp_get_attachment_image( $swp_field, $size = "card-ratio169", $icon = false ); ?></a></div><?php
        }

    }

	// DISPLAY VIDEO ACTUAL
	public function swp_video_entry_actual() {

		?><section class="single-video"><?php

		//// YouTube
		$display_video = get_post_meta( get_the_ID(), "video_youtube", TRUE );
		if( $display_video ) {
		...
		// Vimeo
		$display_video = get_post_meta( get_the_ID(), "video_vimeo", TRUE );
		if( $display_video ) {
	...
	// DISPLAY VIDEO INFO
	public function swp_video_entry_info() {

		?><section class="single-info"><?php
		echo '<div class="item-video">'.do_shortcode( get_post_meta( get_the_ID(), video_embed, TRUE ) ).'</div>';
	...
	// DISPLAY VIDEO GRID TEMPLATE
	/*public function swp_video_related_function() {
	...
	// DISPLAY VIDEO NEXT ITEM
	public function swp_video_next() {
	...
	// GRID
	public function swp_video_related_function_2() {
    	...
    	echo '<div class="feature-pretitle">MORE VIDEOS</div>';
        $more = array(
						'tag-open'		=> '<div class="grid-feature grid-2col feature-related">', // opening container tag here
						'tag-close'		=> '</div>', // closing container tag here
					);
	...
	public function __construct() {
		...
		if( !is_admin() ) {
			...
			add_action( 'genesis_entry_content', array( $this, 'swp_video_entry_pic' ) );
			add_action( 'genesis_entry_content', array( $this, 'swp_video_entry_actual' ) );
			add_action( 'genesis_entry_content', array( $this, 'swp_video_entry_info' ) );
			add_action( 'genesis_entry_content', 'genesis_do_post_content' );
			//add_action( 'genesis_entry_content', array( $this, 'swp_video_related_function' ) );
			add_action( 'genesis_entry_content', array( $this, 'swp_video_related_function_2' ) );
			...
			add_action( 'genesis_before_sidebar_widget_area', array( $this, 'swp_video_next' ) );
